...
class Success extends React.Component<any,any> {
    ...
    public componentWillMount() {
        Axios.get('http://localhost:8083/order/info/'+this.props.match.params.id,{
            headers: {
      ...
      <div>
        ...
        <div>
          <Card
            ...
            style={{ width: 500, marginTop: 20, height: 400,marginLeft: '35%' }}
          ...
          >
            ...
            <p style={{ marginTop:50 }}><Button type='primary' onClick={ () => {this.showConfirm(this.state.order.id)}}>{ this.state.status }</Button><Button style={{ marginLeft:5 }} type='danger'>取消订单</Button></p>
          </Card>
        </div>
      </div>
    );
  ...
  private showConfirm(e:any) {
    confirm({
      title: '支付',
      content: (
        <p><img src="https://i.loli.net/2019/04/09/5cacba762f6ee.png" style={{ width:314, height:390 }}/></p>
      ),
      onOk() {
